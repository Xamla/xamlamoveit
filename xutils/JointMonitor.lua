...
function JointMonitor:getPositions()
  ...
  for i,v in ipairs(self.joint_names) do
    ...
    assert(pos[i] ~= nil, string.format("Position value of joint '%s' is invalid.", v))
  end
...
function JointMonitor:getPositionsTensor()
  local p,l = self:getPositions()
  return torch.DoubleTensor({p}):squeeze(), l
end
...
function JointMonitor:getNextPositionsTensor(timeout)
  self:waitForNextState(timeout)
  return self:getPositionsTensor()
end
