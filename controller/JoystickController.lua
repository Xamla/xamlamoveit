...
function targetTransformation(target_frame, offset, rotation_rpy)
    ...
    --tmp_offset_tf:fromTensor(curr_pose_inv:toTensor() * tmp_offset_tf:toTensor() * curr_pose:toTensor())
    return offset, rotation_rpy
end
...
local function taskStep(delta, vel) -- simple clipping
    local step = vel
    local norm = step:norm()
    if (norm > delta) then
        step = step * (delta / norm)
    end
    return step
end
...
function JoystickController:update()
    ...
    local q_dot, jacobian = self:getStep(deltaForces, detlaTorques, curr_time - self.start_time)
